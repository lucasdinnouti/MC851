...
cd src
iverilog -DSIMULATOR -DPROGRAM='`"full-instruction-set.riscv`"' constants.v alu.v branch.v decoder.v memory/memory_controller.v memory/ram.v memory/rom.v memory/peripherals.v memory/l1.v registers.v ex_mem_pipeline_registers.v id_ex_pipeline_registers.v if_id_pipeline_registers.v mem_wb_pipeline_registers.v forwarding.v atomic.v cpu.v ../test/assert.v ../test/cpu_tb.v && ./a.out
rm a.out
cd ..